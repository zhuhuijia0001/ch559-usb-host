...
BOOL BuildUsbKeyboardPacket(UINT8 *out, UINT8 maxLen, UINT8 *pLen, const UINT8 keyboard[KEYBOARD_LEN])
{
	return BuildPacket(out, maxLen, pLen, ID_USB_KEYBOARD, keyboard, KEYBOARD_LEN);
}

BOOL BuildUsbMousePacket(UINT8 *out, UINT8 maxLen, UINT8 *pLen, const UINT8 mouse[MOUSE_LEN])
{
	return BuildPacket(out, maxLen, pLen, ID_USB_MOUSE, mouse, MOUSE_LEN);
}

BOOL BuildPs2KeyboardPacket(UINT8 *out, UINT8 maxLen, UINT8 *pLen, const UINT8 keyboard[KEYBOARD_LEN])
{
	return BuildPacket(out, maxLen, pLen, ID_PS2_KEYBOARD, keyboard, KEYBOARD_LEN);
}

BOOL BuildPs2MousePacket(UINT8 *out, UINT8 maxLen, UINT8 *pLen, const UINT8 mouse[MOUSE_LEN])
{
